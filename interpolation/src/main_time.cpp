...
int main(int argc, char** argv)
{
    ...
    for(i=0;i<finalPts.size();i++)
    {
     //   if(finalPts[i].z<=2*BALL_RAD)
       //     std::cout<<"z:"<<finalPts[i].z<<", t:"<<time[i]<<'\n';
        ...
        std::cout<<finalPts[i].vx<<' '<<finalPts[i].vy<<' '<<finalPts[i].vz<<' '<<time[i]<<'\n';
    ...
    std::cout<<"\nz:"<<BALL_RAD<<", t1:"<<t1<<", y:"<<spFit1[y].fyCubic(t1)<<", x:"<<spFit1[x].fyCubic(t1);
    std::cout<<"\nz:"<<BALL_RAD<<", t2:"<<t2<<", y:"<<spFit2[y].fyCubic(t2)<<", x:"<<spFit2[x].fyCubic(t2);
    f.close();
    g.close();
    getImpactArea();
    return 0;
