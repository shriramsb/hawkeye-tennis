...
SplineFit::SplineFit(int num, int deg)
{
    n = num;
    degree = deg;
}
...
double SplineFit::calculatePosition()
{
    double dx, dt;
    if(n>=2)
    {
...
double SplineFit::rectangular()
{
    if(n <= maxDataPoints && n>=2)
    {
        position += (_position[n-2]*h[n-2]);
        return position;
    }
    ...
    //    std::cout<<"1\n";
      ...
      //  std::cout<<"A:\n"<<A<<'\n';
       // std::cout<<"3\n";
        ...
        //B->n-1
        B[n-2] = (3.0/h[n-2])*(_position[n-1]-_position[n-2]) -(3.0/h[n-3])*(_position[n-2]-_position[n-3]);
            B.emplace_back(0);
       // std::cout<<"4\n";
        //B->n
...
{   
    ...
    for(int i=1;i<n-1;i++)
    {
        B.emplace_back((3.0/h[i])*(_position[i+1]-_position[i]) -(3.0/h[i-1])*(_position[i]-_position[i-1]));
    }
    B.emplace_back(0);
    //printB();
    Eigen::MatrixXd aInverse(n,n);
...
void SplineFit::calculateAccuratePosition()
{
    ...
    std::vector<double> positionNew,positionOld = _position;
