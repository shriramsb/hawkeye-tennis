...
void Camera::learnBg(){
	...
	while(end - cap.get(CV_CAP_PROP_POS_MSEC) > 1000){
		cap.read(temp1);
		remap(temp1, temp, mapx, mapy, INTER_LINEAR);
		medianBlur(temp, temp, kernel_size);
		pMOG2->operator()(temp, temp1);							//updating background model
		temp.release();
		temp1.release();
	}
	cout << "Successfully completed" << endl;
	//cap.set(CV_CAP_PROP_POS_FRAMES,0);							//set the pointer back to start to sync later
}
